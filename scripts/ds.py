...
def cmd_list():
    ...
    if not entities:
        ...
        return
    print(f"{'ID':<4} {'status':<12} {'retries':<9} {'lease_expires_at':<21} | {'updated_at':<21} {'duration':<8} sleep")
    print("-" * 49 + "-+-" + "-" * 35)
    for e in entities:
        lease    = str(e.get('lease_expires_at', '-'))[:19]
        updated  = str(e.get('updated_at', '-'))[:19]
        duration = f"{e['duration_sec']:.1f}" if 'duration_sec' in e else '-'
        retries  = f"{e.get('retry_count', 0)}/{MAX_GRADE_RETRIES}"
        sleep    = str(e.get('sleep_time', '-'))
        print(f"{e.key.name:<4} {e.get('status', '?'):<12} {retries:<9} {lease:<21} | {updated:<21} {duration:<8} {sleep}")
